...
| Automation *(future)* | When should execution happen? | `ai_os.automation` |
...
- `knowledge` → `decision`, `memory`, or `agent`
- `memory` → `knowledge`, `decision`, or `agent`
- `decision` → `memory` or `agent`
...
layers = ['knowledge', 'decision', 'memory', 'agent']
ROOT = Path('src/ai_os')
for layer in layers:
    deps = set()
